...
async function exit(code, forceSync = false) {
	channel.send({
		type: 'debug',
		message: `exit called with code ${code}`
	})
	...
	if (!forceSync) {
		channel.send({
			type: 'debug',
			message: `waiting to flush channel...`
		})
		...
		channel.send({
			type: 'debug',
			message: `flushed channel successfully!`
		})
	}

	channel.send({
		type: 'debug',
		message: `calling realExit with code ${code}`
	})
...
const run = async options => {
	...
	runner.on('finish', async () => {
		try {
			channel.send({ type: 'debug', message: 'saving snapshot state...'})

			const {touchedFiles} = await runner.saveSnapshotState();

			channel.send({ type: 'debug', message: 'saved snapshot state!'})
			if (touchedFiles) {
		...
		channel.send({ type: 'debug', message: 'finalizing and exiting with code 0 (scheduled)...'})

		nowAndTimers.setImmediate(() => {
			channel.send({ type: 'debug', message: 'finalizing and exiting with code 0 (immediate)...'})
			...
			channel.send({ type: 'debug', message: 'exiting process'})
	...
	const load = async ref => {
		for (const provider of providers) {
			if (provider.canLoad(ref)) {
				channel.send({ type: 'debug', message: "loading provider...", provider })
		...
		for (const extension of extensionsToLoadAsModules) {
			if (ref.endsWith(`.${extension}`)) {
				channel.send({ type: 'debug', message: "loading extension...", extension })
				return import(pathToFileURL(ref));
			}
		}

		// We still support require() since it's more easily monkey-patched.
		channel.send({ type: 'debug', message: "loading ref directly...", ref })
		return require(ref);
	};
	...
	try {
		...
		channel.send({ type: 'debug', message: `loading testPath: ${testPath}` })

		await load(testPath);

		channel.send({ type: 'debug', message: `finished loading testPath: ${testPath}` })

		if (flags.loadedMain) {
			channel.send({ type: 'debug', message: `unreferencing channel` })
			...
			channel.send({ type: 'debug', message: `unreferenced channel` })
		} else {
			channel.send({type: 'missing-ava-import'});
			exit(1);
		}
	} catch (error) {
		channel.send({ type: 'debug', message: { error, msg: "Uncaught exception" }})

		channel.send({type: 'uncaught-exception', err: serializeError('Uncaught exception', true, error, runner.file)});
		exit(1);
	}

  channel.send({ type: "debug", message: `finished run function` })
};
