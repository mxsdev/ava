...
const debugMsg = (message) =>
	channel.send({ type: 'debug', message })
...
const run = async options => {
	...
	try {
		...
		debugMsg(`loading testPath: ${testPath}`)

		await load(testPath);

		debugMsg(`finished loading testPath: ${testPath}`)

		if (flags.loadedMain) {
			debugMsg(`unreferencing channel`)
			...
			debugMsg(`unreferenced channel`)
		} else {
			channel.send({type: 'missing-ava-import'});
			exit(1);
		}
	} catch (error) {
		debugMsg({ error, msg: "Uncaught exception" })
