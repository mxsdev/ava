...
export default class Runner extends Emittery {
	...
	async runTest(task, contextRef) {
		...
		this.emit('stateChange', {
			type: 'debug',
			message: `running before each hooks...`,
			title: task.title,
			file: this.file,
		})
		...
		this.emit('stateChange', {
			type: 'debug',
			message: `running after each hooks...`,
			title: task.title,
			file: this.file,
		})
	...
	async start() { // eslint-disable-line complexity
		...
		try {
			this.emit('stateChange', {
				type: 'debug',
				message: `waiting for all tests to finish....`,
				file: this.file,
			})
			...
			this.emit('stateChange', {
				type: 'debug',
				message: `running after.always hooks...`,
				file: this.file,
			})
			...
			this.emit('stateChange', {
				type: 'debug',
				message: `ran after.always hooks!`,
				file: this.file,
			})
