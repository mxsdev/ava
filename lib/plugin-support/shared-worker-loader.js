			...
			parentPort.on('message', async message => {
				if (message.type === 'register-test-worker') {
					...
					parentPort.postMessage({
						type: 'debug',
						message: `Registering test worker`,
						numTestWorkers: activeTestWorkers.size,
						id
					})
				...
				if (message.type === 'deregister-test-worker') {
					...
					parentPort.postMessage({
						type: 'debug',
						message: `Deregistering test worker`,
						numTestWorkers: activeTestWorkers.size,
						id
					})
					...
					parentPort.postMessage({
						type: 'debug',
						message: `Running teardown functions...`,
						id
					})
					...
					parentPort.postMessage({
						type: 'debug',
						message: `Ran teardown functions!`,
						id
					})
