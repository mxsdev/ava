...
function launchWorker(filename, initialData) {
	...
	console.log({
		type: 'debug',
		message: `launching new shared worker`,
		initialData,
		filename,
	})
	...
	console.log({
		type: 'debug',
		message: `Launched new shared worker`,
		id: worker.threadId,
		initialData,
		filename,
		size: launchedWorkers.size,
	})

	worker.once('exit', () => {
		console.log({
			type: 'debug',
			message: 'Shared worker exited!',
			id: worker.threadId,
		})
...
export async function observeWorkerProcess(fork, runStatus) {
	...
	fork.onConnectSharedWorker(async ({filename, initialData, port, signalError, stack}) => {
		console.log({
			type: 'debug',
			message: `Shared worker connected, launch requested`,
			stack,
		})
