...
const createWorker = (options, execArgv) => {
	...
	if (options.workerThreads) {
		...
		close = async () => {
			try {
				console.log(`running close for worker ${worker.threadId}...`)
				await starting;
				console.log(`finished polling for starting for worker ${worker.threadId}...`)
				await worker.terminate();
				console.log(`closed worker with pid ${worker.pid}!`)
			} finally {
				// No-op
			}
		};
		...
		worker.on('message', message => {
			...
			switch (message.ava.type) {
				...
				case 'shared-worker-connect': {
					const {channelId, filename, initialData, port, stack} = message.ava;
				...
				case "debug": {
          emitStateChange({ ...message.ava, id: worker.threadId });
          break;
        }
