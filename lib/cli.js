...
const FLAGS = {
	...
	"node-arguments": {
		coerce: coerceLastValue,
		description:
			"Additional Node.js arguments for launching worker processes (specify as a single string)",
		type: "string",
	},
	serial: {
		alias: "s",
...
export default async function loadCli() {
	...
	try {
		...
		} = yargs(hideBin(process.argv)).help(false).version(false);
		...
		if (loaded.unsupportedFiles.length > 0) {
			console.log(
				chalk.magenta(
					`  ${
						figures.warning
					} AVA does not support JSON config, ignoring:\n\n    ${loaded.unsupportedFiles.join(
						"\n    "
					)}`
				)
			);
		...
		if (
			conf.configFile &&
			path.basename(conf.configFile) !==
				path.relative(conf.projectDir, conf.configFile)
		) {
			console.log(
				chalk.magenta(
					`  ${figures.warning} Using configuration from ${conf.configFile}`
				)
			);
		}
	} catch (error) {
	...
	const { argv } = yargs(hideBin(process.argv))
		...
		.options({
			...
			config: {
				description:
					"Specific JavaScript file for AVA to read its config from, instead of using package.json or ava.config.* files",
			},
		})
		.command(
			"* [<pattern>...]",
				...
				yargs
					...
					.options({
						...
						host: {
							default: "127.0.0.1",
							description:
								"Address or hostname through which you can connect to the inspector",
							type: "string",
						},
						port: {
							default: 9229,
					...
					.positional("pattern", {
						demand: true,
						describe:
							"Glob pattern to select a single test file to debug, optionally suffixed with a colon and comma-separated numbers and/or ranges identifying the 1-based line(s) of specific tests to run",
						type: "string",
					}),
			(argv) => {
				debug = {
	...
	if (combined.color !== false) {
		const {
			supportsColor: { level },
		} = await import("chalk"); // eslint-disable-line unicorn/import-style
	...
	if (confError) {
		if (confError.cause) {
			exit(
				`${confError.message}\n\n${chalk.gray(
					confError.cause?.stack ?? confError.cause
				)}`
			);
		} else {
	...
	if (debug !== null) {
		...
		if (combined.timeout) {
			console.log(
				chalk.magenta(
					`  ${figures.warning} The timeout option has been disabled to help with debugging.`
				)
			);
		}
	}
	...
	if (
		Object.hasOwn(combined, "concurrency") &&
		(!Number.isInteger(combined.concurrency) || combined.concurrency < 0)
	) {
		exit(
	...
	if (
		Object.hasOwn(conf, "sortTestFiles") &&
		typeof conf.sortTestFiles !== "function"
	) {
	...
	if (!combined.tap && Object.keys(experiments).length > 0) {
		console.log(
			chalk.magenta(
				`  ${figures.warning} Experiments are enabled. These are unsupported and may change or be removed at any time.`
			)
		);
	}
	...
	try {
		projectPackageObject = JSON.parse(
	...
	if (Object.hasOwn(conf, "typescript")) {
		...
		try {
			const {
				...
				level,
				main,
			} = await providerManager.typescript(projectDir, { fullConfig: conf });
			providers.push({
				level,
				protocol,
	...
	let moduleTypes;
	try {
		moduleTypes = normalizeModuleTypes(
			conf.extensions,
			defaultModuleType,
			experiments
		);
	...
	try {
		globs = normalizeGlobs({
			files: conf.files,
			ignoredByWatcher: conf.watchMode?.ignoreChanges,
			extensions,
			providers,
		});
	} catch (error) {
	...
	let nodeArguments;
	try {
		nodeArguments = normalizeNodeArguments(
			conf.nodeArguments,
			argv["node-arguments"]
		);
	} catch (error) {
	...
	const filter = input
		...
		.map(({ pattern, ...rest }) => ({
			pattern: normalizePattern(
				path.relative(projectDir, path.resolve(process.cwd(), pattern))
			),
			...rest,
		}));
	...
	const api = new Api({
		...
		snapshotDir: combined.snapshotDir
			? path.resolve(projectDir, combined.snapshotDir)
			: null,
	...
	if (argv.watch) {
		if (
			Object.hasOwn(conf, "watchMode") &&
			Object.hasOwn(conf.watchMode, "implementation")
		) {
			if (conf.watchMode.implementation === "ava5+chokidar") {
				...
				exit(
					"The ’watchMode.implementation’ option must be set to “ava5+chokidar”"
				);
			}
		} else {
