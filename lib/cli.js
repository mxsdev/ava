...
export default async function loadCli() { // eslint-disable-line complexity
	...
	if (combined.reporter) {
		reporter = new combined.reporter({
			extensions: globs.extensions,
			projectDir,
			reportStream: process.stdout,
			stdStream: process.stderr,
			defaultReporter: (await import('./reporters/default.js')).default,
			tapReporter: (await import('./reporters/tap.js')).default,
		})
	} else if (combined.tap && !argv.watch && debug === null) {
	...
	if (argv.watch) {
		if (Object.hasOwn(conf, 'watchMode') && Object.hasOwn(conf.watchMode, 'implementation')) {
